...
import HeroLiveWords from "@/components/hero-live-words";
...
export default function Home() {
  return <main className="min-h-screen overflow-hidden bg-white text-[#090d1f]">
    ...
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#fff8ff_0%,#ffffff_38%,#f2f7ff_70%,#f5f2ff_100%)]">
      ...
      <div className="relative mx-auto grid max-w-[1380px] gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[.92fr_1.08fr] lg:items-center lg:py-20 xl:py-24">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 rounded-full border border-violet-100 bg-white/90 px-4 py-2 text-sm font-black text-violet-700 shadow-sm"><span className="h-2 w-2 rounded-full bg-violet-500"/>Made for job seekers in India</div>
          <h1 className="mt-7 text-[3.3rem] font-black leading-[.94] tracking-[-.067em] sm:text-6xl xl:text-[5.7rem]">Find jobs that fit.<br/><span className="bg-gradient-to-r from-violet-600 via-fuchsia-500 to-indigo-500 bg-clip-text text-transparent">Apply with confidence.</span></h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">Find roles, see why they match, prepare the right resume and track every application — all in one place.</p>
          <HeroLiveWords />
          <div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/jobs" className="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-7 py-4 text-center font-black text-white shadow-xl shadow-violet-200/70 transition hover:-translate-y-0.5">Find jobs →</Link><Link href="/resume" className="rounded-xl border border-violet-100 bg-white/90 px-7 py-4 text-center font-black shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">Build my resume</Link></div>
          <div className="mt-9 flex flex-wrap gap-3 text-sm font-black text-slate-600">{["See your match","Build better resumes","Track applications","Know your next step"].map((item)=><span key={item} className="rounded-full border border-white bg-white/85 px-4 py-2 shadow-sm">✓ {item}</span>)}</div>
        </div>
        <HeroProductVisual />
      </div>
    </section>
    ...
    <section className="border-y border-violet-100/70 bg-white py-5">
      <div className="mx-auto flex max-w-[1380px] flex-wrap items-center justify-center gap-x-8 gap-y-3 px-5 text-sm font-black text-slate-500 sm:px-8 lg:justify-between">{["Find a job","See your match","Fix your resume","Track your progress","Know what comes next"].map((x,i)=><div key={x} className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-violet-50 text-[10px] text-violet-700">0{i+1}</span>{x}</div>)}</div>
    </section>
    ...
    <section className="relative overflow-hidden bg-[linear-gradient(135deg,#fff9fe_0%,#f8f7ff_50%,#eefbff_100%)] py-24 sm:py-28">
      ...
      <div className="relative mx-auto grid max-w-[1380px] gap-10 px-5 sm:px-8 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
        <div>
          <p className="text-xs font-black tracking-[.2em] text-violet-600">WHY JOBCRAFT</p>
          <h2 className="mt-4 text-4xl font-black tracking-[-.05em] sm:text-6xl">Less searching. Better applying.</h2>
          <p className="mt-5 max-w-xl text-lg leading-8 text-slate-600">JobCraft keeps the important parts of your job search together so you can spend less time guessing and more time moving forward.</p>
          <Link href="/?auth=signup" scroll={false} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-[#090d1f] px-6 py-3.5 font-black text-white shadow-lg transition hover:-translate-y-0.5">Start free <span>→</span></Link>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          {[["01","Find better jobs","Search by role, salary, city, experience and work mode."],["02","See why you match","Know what fits and what is missing before you apply."],["03","Use the right resume","Keep different resume versions ready for different roles."],["04","Stay on track","See what is saved, applied, interviewing or done."]].map(([n,title,text],i)=><article key={n} className={`rounded-[28px] border p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${i===0?"border-violet-100 bg-violet-50/80":i===1?"border-sky-100 bg-sky-50/80":i===2?"border-fuchsia-100 bg-fuchsia-50/70":"border-emerald-100 bg-emerald-50/70"}`}><div className="flex items-center justify-between"><span className="text-xs font-black tracking-[.16em] text-slate-400">{n}</span><span className="h-2.5 w-2.5 rounded-full bg-violet-500"/></div><h3 className="mt-7 text-2xl font-black">{title}</h3><p className="mt-3 leading-7 text-slate-600">{text}</p></article>)}
        </div>
      </div>
    </section>
    ...
    <section className="bg-white px-5 py-20 sm:px-8">
      <div className="relative mx-auto max-w-[1380px] overflow-hidden rounded-[36px] bg-[linear-gradient(120deg,#6d28d9_0%,#4f46e5_48%,#0ea5e9_100%)] px-7 py-14 text-white shadow-[0_35px_100px_rgba(79,70,229,.24)] sm:px-12 lg:flex lg:items-center lg:justify-between lg:py-16">
        ...
        <div className="relative"><p className="text-xs font-black tracking-[.2em] text-violet-100">READY WHEN YOU ARE</p><h2 className="mt-4 max-w-3xl text-4xl font-black tracking-[-.045em] sm:text-5xl">One profile. Better applications.</h2><p className="mt-4 max-w-2xl text-lg leading-8 text-violet-100">Set up your profile once, then use JobCraft to find, prepare and track your next opportunities.</p></div>
