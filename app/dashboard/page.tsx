...
export default async function DashboardPage() {
  ...
  const [
    ...
    { data: applications },
    { data: jobs },
    { count: freshRoleCount },
  ] = await Promise.all([
    supabase
      .from("profiles")
      .select("full_name,city,headline,experience_years,target_roles,skills,preferred_work_modes")
      .eq("id", user.id)
      .maybeSingle(),
    supabase.from("resumes").select("id", { count: "exact", head: true }).eq("user_id", user.id),
    supabase
      .from("applications")
      .select("id,status,created_at,updated_at,jobs(id,title,company,location,work_mode)")
      .eq("user_id", user.id)
      .order("updated_at", { ascending: false }),
    supabase
      .from("jobs")
      .select(
        "id,title,company,location,work_mode,salary_min_lpa,salary_max_lpa,experience_min,skills,source,posted_at",
      )
      .eq("is_active", true)
    ...
    supabase
      ...
      .neq("apply_url_status", "dead")
      .gte("first_seen_at", last24Hours),
  ]);
  ...
  const rankedJobs: RankedJob[] = ((jobs ?? []) as JobRecord[])
    .map((job) => ({
      ...job,
    ...
    .sort((a, b) => {
      ...
      return Math.abs(evidenceGap) > 0.25 ? evidenceGap : b.match.score - a.match.score;
    });

  const recommendations = rankedJobs.slice(0, 4);
  const applicationItems = (applications ?? []) as unknown as ApplicationRecord[];
  const strongMatchCount = rankedJobs.filter(
    (job) => job.match.score >= 75 && job.match.evidenceCoverage >= 0.5,
  ).length;
  const savedCount = applicationItems.filter((item) => item.status === "Saved").length;
  const motionCount = applicationItems.filter((item) =>
      ...
      <div className="jc-content-wrap">
        <section className="jc-dashboard-head">
          <div>
            ...
            <p className="jc-page-copy mt-3">
              {freshRoleCount ?? 0} new role{freshRoleCount === 1 ? "" : "s"} in the last 24 hours ·{" "}
              {strongMatchCount} strong match{strongMatchCount === 1 ? "" : "es"} in your current
              shortlist.
            </p>
          </div>
        ...
        <section className="jc-stats-grid" aria-label="Career search summary">
          <StatCard
            label="New roles · 24h"
            value={freshRoleCount ?? 0}
            note="fresh live opportunities"
            icon="↗"
          />
          <StatCard
            label="Strong matches"
            value={strongMatchCount}
            note="75%+ with enough evidence"
            icon="✣"
          />
        ...
        <section className="jc-card mt-6 p-5 sm:p-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="jc-eyebrow">ONE CLEAR JOB JOURNEY</p>
              <h2 className="jc-section-title">From discovery to interview prep</h2>
              <p className="jc-section-subtitle">
                JobCraft keeps the next useful action visible instead of throwing twenty shiny
                buttons at you.
              </p>
            </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            ].map(([step, title, href, text]) => (
              <Link
                key={step}
                href={href}
                className="rounded-[15px] bg-[#efede7] p-4 text-inherit no-underline transition hover:-translate-y-0.5 hover:bg-[#e8ece7]"
              >
        ...
        <section className="jc-dashboard-grid mt-6">
          <div className="jc-card jc-section-card">
            <div className="jc-section-head">
              <div>
                ...
                <p className="jc-section-subtitle">
                  A short list of opportunities that fit where you&apos;re headed.
                </p>
              </div>
            ...
            <div className="jc-role-list">
              {recommendations.length ? (
                ...
                <div className="py-10 text-sm leading-6 text-[#6f887f]">
                  Add target roles and skills to your profile, then JobCraft will rank the strongest
          ...
          <div className="jc-dark-card jc-coming-card">
            <div className="jc-section-head">
              <div>
                <p className="jc-eyebrow">KEEP THE MOMENTUM</p>
                <h2 className="jc-section-title">Coming up</h2>
              </div>
            ...
            <div className="jc-upcoming-list">
              {upcoming.length ? (
                upcoming.map((item) => {
                  const href =
                    item.status === "Interview" && item.jobs?.id
                      ? `/career-assistant?mode=interview&jobId=${item.jobs.id}`
                      : "/applications";
                  return (
                    <Link
                      href={href}
                      key={item.id}
                      className="jc-upcoming-item text-inherit no-underline"
                    >
                      <span className="jc-upcoming-badge">
                        {companyInitials(item.jobs?.company || "JC").slice(0, 1)}
                      </span>
                      <span className="jc-upcoming-copy">
                        ...
                        <span>
                          {item.jobs?.company || "Application"} · {item.jobs?.title || "Role"}
                        </span>
                        <span className="jc-upcoming-date">
                          {item.status === "Interview"
                            ? "Open job-specific prep →"
                ...
                <div className="pt-3">
                  ...
                  <p className="mt-2 text-xs leading-6 text-[#9bb3aa]">
                    Save a role or move an application forward and the next action will appear here.
                  </p>
                </div>
              )}
            </div>
            <Link
              href="/applications"
              className="mt-5 inline-block text-sm font-extrabold text-[#f49a48] no-underline"
            >
        ...
        <section className="jc-card jc-activity-card">
          <div className="jc-section-head">
            <div>
              <p className="jc-eyebrow">A CLEAR TRAIL</p>
              <h2 className="jc-section-title">Recent activity</h2>
            </div>
            <Link href="/applications" className="jc-text-link">
              See applications ↗
            </Link>
          </div>
          <div className="jc-activity-grid">
            {applicationItems.slice(0, 4).map((item) => (
              <div key={item.id} className="jc-activity-item">
                ...
                <span>
                  {item.jobs?.company || "JobCraft"} · {item.jobs?.title || "Application"} ·{" "}
                  {new Date(item.updated_at).toLocaleDateString("en-IN", {
                    day: "2-digit",
                    month: "short",
                  })}
            ...
            {!applications?.length && (
              <>
                <div className="jc-activity-item">
                  <b>Profile signal ready</b>
                  <span>{profileStrength}% complete · strengthen it from My profile</span>
                </div>
                <div className="jc-activity-item">
                  <b>{hasResume ? "Resume workspace ready" : "Resume still to add"}</b>
                  <span>
                    {hasResume
                      ? `${resumeCount} saved resume version${resumeCount === 1 ? "" : "s"}`
                      : "Build or upload your first factual resume"}
                  </span>
                </div>
              </>
            )}
...
function GuestWorkspace() {
  ...
  return (
    <WorkspaceShell
      active="workspace"
      name="Your profile"
      headline="Log in to activate your workspace"
      strength={0}
      authenticated={false}
    >
      <div className="jc-content-wrap">
        <section className="jc-dashboard-head">
          <div>
            ...
            <p className="jc-page-copy max-w-2xl">
              Discover live jobs, understand your fit, tailor a factual resume, track applications
              and prepare for interviews in one career workspace.
            </p>
          </div>
          <Link href="/dashboard?auth=login" scroll={false} className="jc-button-primary">
            Log in to JobCraft <span>→</span>
          </Link>
        </section>
        ...
        <section className="jc-dashboard-grid">
          <div className="jc-card jc-section-card">
            <div className="jc-section-head">
              <div>
                ...
                <p className="jc-section-subtitle">
                  Explore the product before signing in. Personal data appears only after
                  authentication.
                </p>
              </div>
            </div>
            <div className="jc-role-list">
              {[
                [
                  "JM",
                  "Discover & match",
                  "Search live roles and see evidence-aware fit signals.",
                  "/jobs",
                ],
                [
                  "RT",
                  "Tailor your resume",
                  "Prepare a role-specific version from real evidence.",
                  "/resume/tailor",
                ],
                [
                  "AP",
                  "Track applications",
                  "Move saved roles through interview and offer.",
                  "/applications",
                ],
                [
                  "IP",
                  "Prepare for interviews",
                  "Use the actual job to focus your preparation.",
                  "/career-assistant",
                ],
                ...
                <Link key={title} href={href} className="jc-role-row">
                  ...
                  <span className="jc-role-meta">
                    <span className="jc-match-pill">Explore</span>
                  </span>
                  <span aria-hidden="true">›</span>
                </Link>
              ))}
          ...
          <div className="jc-dark-card jc-coming-card">
            <div className="jc-section-head">
              <div>
                <p className="jc-eyebrow">THE FLOW</p>
                <h2 className="jc-section-title">Know the next move</h2>
              </div>
            </div>
            <div className="mt-6 space-y-4 text-sm leading-6 text-[#b5c7c0]">
              <p>
                <b className="text-white">1. Find</b>
                <br />
                Discover fresh roles worth reviewing.
              </p>
              <p>
                <b className="text-white">2. Understand</b>
                <br />
                See why the role fits and what needs checking.
              </p>
              <p>
                <b className="text-white">3. Prepare</b>
                <br />
                Tailor your resume and application materials.
              </p>
              <p>
                <b className="text-white">4. Move</b>
                <br />
                Apply, track and prepare for the conversation.
              </p>
            </div>
            <Link
        ...
        <section className="jc-card jc-activity-card">
          <div className="jc-section-head">
            <div>
              <p className="jc-eyebrow">PRIVATE BY DEFAULT</p>
              <h2 className="jc-section-title">Your data appears only after login</h2>
            </div>
          </div>
          <div className="jc-activity-grid">
            <div className="jc-activity-item">
              <b>Profile signal</b>
              <span>Skills, target roles, experience, city and work preferences.</span>
            </div>
            <div className="jc-activity-item">
              <b>Career documents</b>
              <span>Resumes, certificates, tailored versions and cover letters.</span>
            </div>
            <div className="jc-activity-item">
              <b>Application history</b>
              <span>Saved roles, applications, interviews and offers.</span>
            </div>
            <div className="jc-activity-item">
              <b>Match insights</b>
              <span>Only known job data and profile evidence contribute to fit signals.</span>
            </div>
          </div>
        </section>
      </div>
  ...
  return (
    <div className="jc-card jc-stat-card">
      <div className="jc-stat-top">
        <span>{label}</span>
        <span className="jc-stat-icon">{icon}</span>
      </div>
...
function LockedStat({ label, note }: { label: string; note: string }) {
  return (
    <div className="jc-card jc-stat-card">
      ...
      <div className="jc-stat-value">—</div>
      <div className="jc-stat-note">{note}</div>
    </div>
  );
}
