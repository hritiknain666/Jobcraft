...
const allowedStatuses = ["Saved", "Applied", "Screening", "Interview", "Offer", "Rejected"] as const;
...
export async function saveApplication(formData: FormData) {
  ...
  if (existing) {
    await supabase.from("applications").update({ status, updated_at: new Date().toISOString() }).eq("id", existing.id).eq("user_id", user.id);
  } else {
...
export async function updateApplicationStatus(formData: FormData) {
  ...
  await supabase.from("applications").update({ status, updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", user.id);
  revalidatePath("/applications");
}
