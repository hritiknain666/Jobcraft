...
export default async function ApplicationsPage() {
  ...
  const [{ data: applications }, { data: profile }] = await Promise.all([
    supabase
      .from("applications")
      .select(
        "id,status,created_at,updated_at,jobs(id,title,company,location,work_mode,salary_min_lpa,salary_max_lpa)",
      )
      .eq("user_id", user.id)
    ...
    >
      <div className="jc-content-wrap">
        ...
        <div className="jc-pipeline-pills">
          ...
          <span className="jc-pipeline-pill">
            Applied · {(counts.Applied ?? 0) + (counts.Screening ?? 0)}
          </span>
        ...
        <section className="jc-board" aria-label="Application pipeline">
          ...
          <BoardColumn
            title="Interview"
            note="Conversations in motion"
            items={items.filter((item) => item.status === "Interview")}
          />
          <BoardColumn
            title="Offer"
            note="A good problem to have"
            items={items.filter((item) => item.status === "Offer")}
          />
        </section>
        ...
        {closed.length ? (
          <section className="jc-card mt-7 p-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="jc-eyebrow">CLOSED LOOP</p>
                <h2 className="jc-section-title">Past applications</h2>
              </div>
              <span className="text-xs font-bold text-[#789087]">{closed.length} closed</span>
            </div>
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {closed.map((item) => (
                <ApplicationCard key={item.id} item={item} compact />
              ))}
            </div>
          </section>
        ) : null}
...
function BoardColumn({
  ...
  return (
    <div className="jc-board-column">
      <div className="jc-board-title">
        <b>{title}</b>
        <span className="jc-board-count">{items.length}</span>
      </div>
      <p className="jc-board-note">{note}</p>
      {items.length ? (
        items.map((item) => <ApplicationCard key={item.id} item={item} />)
      ) : (
        <div className="mt-6 flex min-h-[190px] items-center justify-center rounded-[18px] border border-dashed border-[#d4cec2] text-center text-xs text-[#789087]">
          Nothing here yet
        </div>
      )}
    </div>
    ...
    <article className={`jc-application-card ${compact ? "mt-0" : ""}`}>
      <div className="jc-application-company">
        ...
        <span className="min-w-0">
          <Link href={job?.id ? `/jobs/${job.id}` : "/jobs"}>
            <b>{job?.title || "Job"}</b>
          </Link>
          <span>
            {job?.company || "JobCraft"} · {job?.location || "India"}
          </span>
        </span>
      </div>
      <div className="jc-application-actions">
        ...
        {item.status === "Interview" && job?.id ? (
          <Link
            href={`/career-assistant?mode=interview&jobId=${job.id}`}
            className="jc-mini-button mt-2 inline-flex w-full justify-center no-underline"
          >
        ...
        <div className="flex items-center justify-between gap-3 text-[10px] text-[#789087]">
          <span>
            {item.status === "Interview"
              ? "◷ Job-specific prep ready"
              : item.status === "Saved"
                ? "◷ Tailor resume"
                : `Updated ${new Date(item.updated_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}`}
          </span>
          <form action={deleteApplication}>
            <input type="hidden" name="id" value={item.id} />
            <button className="font-bold text-[#8b6d67]">Remove</button>
          </form>
        </div>
      </div>
    </article>
  ...
  const demo = [
    {
      title: "Saved",
      note: "Worth a closer look",
      item: ["DA", "Data Analyst", "Analytics Co. · Bengaluru"],
    },
    {
      title: "Applied",
      note: "You made the move",
      item: ["BA", "Business Analyst", "Fintech Co. · Gurugram"],
    },
    {
      title: "Interview",
      note: "Conversations in motion",
      item: ["BI", "Power BI Developer", "Product Co. · Hyderabad"],
    },
        ...
        <section className="jc-discover-head">
          <div>
            ...
            <p className="jc-page-copy">
              Save roles, track progress, prepare for conversations and keep the next action
              visible.
            </p>
          </div>
          <Link href="/applications?auth=signup" scroll={false} className="jc-button-primary">
            Start your plan →
          </Link>
        </section>
        <div className="jc-pipeline-pills">
          <span className="jc-pipeline-pill is-active">All · 3</span>
          <span className="jc-pipeline-pill">Saved · 1</span>
          <span className="jc-pipeline-pill">Applied · 1</span>
          <span className="jc-pipeline-pill">Interview · 1</span>
          <span className="jc-pipeline-pill">Offer · 0</span>
        </div>
        <section className="jc-board">
          {demo.map((column) => (
            <div key={column.title} className="jc-board-column">
              <div className="jc-board-title">
                <b>{column.title}</b>
                <span className="jc-board-count">{column.item ? 1 : 0}</span>
              </div>
              <p className="jc-board-note">{column.note}</p>
              {column.item ? (
                <div className="jc-application-card">
                  <div className="jc-application-company">
                    <span className="jc-company-dot">{column.item[0]}</span>
                    <span>
                      <b>{column.item[1]}</b>
                      <span>{column.item[2]}</span>
                    </span>
                  </div>
                  <div className="jc-application-actions">
                    <span className="text-[10px] text-[#789087]">
                      Create an account to manage this stage.
                    </span>
                  </div>
                </div>
              ) : (
                <div className="mt-6 flex min-h-[190px] items-center justify-center rounded-[18px] border border-dashed border-[#d4cec2] text-xs text-[#789087]">
                  Nothing here yet
                </div>
              )}
            </div>
