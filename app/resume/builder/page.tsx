...
export default async function ResumeBuilderPage({ searchParams }: { searchParams: Promise<{ id?: string; saved?: string; error?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f7f8fc] text-[#090d1f]">
    <header className="border-b border-slate-200/70 bg-white"><div className="mx-auto flex max-w-[1380px] items-center justify-between px-5 py-4 sm:px-8"><Link href="/" className="flex items-center gap-3 font-black"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#090d1f] text-sm text-white">JC</span><span className="text-xl">Job<span className="text-violet-600">Craft</span></span></Link><div className="flex items-center gap-2"><Link href="/certificates" className="hidden rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold sm:block">Certificates</Link><Link href="/resume" className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold">← My resumes</Link></div></div></header>
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
      <form action={saveResumeDraft} className="space-y-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,.05)] sm:p-8"><input type="hidden" name="id" value={resume?.id ?? ""}/>
        ...
        <div className="border-t border-slate-100 pt-2"><SectionTitle step="03" title="Evidence" text="Education, experience, projects and credentials should prove the skills above."/></div>
        ...
        <div className="rounded-2xl border border-violet-100 bg-violet-50/60 p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h3 className="font-black text-violet-950">Certificate library</h3><p className="mt-1 text-sm leading-6 text-violet-900">Choose saved credentials to show on this resume. The uploaded proof file stays private.</p></div><Link href="/certificates" className="text-sm font-black text-violet-700">Manage certificates →</Link></div>
          <div className="mt-4 space-y-3">{certificates?.length ? certificates.map((certificate:any) => <label key={certificate.id} className="flex cursor-pointer items-start gap-3 rounded-xl border border-violet-100 bg-white p-4"><input type="checkbox" name="certificate_ids" value={certificate.id} defaultChecked={selectedCertificateIds.includes(certificate.id)} className="mt-1"/><span><b className="block">{certificate.name}</b><span className="mt-1 block text-sm text-slate-500">{certificate.issuer}{certificate.issue_date ? ` · ${new Date(certificate.issue_date).getFullYear()}` : ""}</span></span></label>) : <div className="rounded-xl border border-dashed border-violet-200 bg-white p-4 text-sm text-slate-600">No saved certificates yet. <Link href="/certificates" className="font-black text-violet-700">Add your first certificate.</Link></div>}</div>
        </div>

        <Area label="Other certifications or licences" name="certifications" defaultValue={d.certifications ?? ""} placeholder="Use this only for credentials not saved in your certificate library."/>
      ...
      <aside className="xl:sticky xl:top-24 xl:self-start"><section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,.05)] sm:p-8"><div className="flex items-center justify-between"><div><p className="text-xs font-black tracking-[.14em] text-violet-600">ATS PREVIEW</p><p className="mt-1 text-sm text-slate-500">Preview updates after you save.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700">Simple layout</span></div><div className="mt-7 border border-slate-200 bg-white p-7 shadow-sm"><h2 className="text-3xl font-black tracking-tight">{d.full_name || "Your Name"}</h2><p className="mt-1 font-semibold text-slate-600">{d.headline || "Professional headline"}</p><p className="mt-2 text-sm text-slate-500">{[d.email,d.phone,d.city].filter(Boolean).join(" • ") || "email • phone • city"}</p><Preview title="Summary" text={d.summary}/><Preview title="Skills" text={(d.skills??[]).join(" • ")}/><Preview title="Experience" text={d.experience}/><Preview title="Projects" text={d.projects}/><Preview title="Education" text={d.education}/>{selectedCertificates.length ? <div className="mt-6 border-t border-slate-200 pt-4"><h3 className="text-xs font-black uppercase tracking-[.12em]">Certifications</h3><div className="mt-3 space-y-3">{selectedCertificates.map((certificate:any)=><div key={certificate.id}><p className="text-sm font-black">{certificate.name}</p><p className="mt-1 text-xs text-slate-500">{certificate.issuer}{certificate.issue_date?` · ${new Date(certificate.issue_date).getFullYear()}`:""}{certificate.credential_id?` · ID ${certificate.credential_id}`:""}</p>{certificate.credential_url&&<p className="mt-1 break-all text-xs text-violet-700">{certificate.credential_url}</p>}</div>)}</div>{d.certifications&&<p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{d.certifications}</p>}</div> : <Preview title="Certifications" text={d.certifications}/>}</div></section>
      <section className="mt-5 rounded-[24px] border border-violet-100 bg-violet-50 p-6"><p className="text-sm font-black text-violet-950">JobCraft resume rules</p><ul className="mt-3 space-y-2 text-sm leading-6 text-violet-900"><li>• Keep information factual.</li><li>• Prefer measurable outcomes when you genuinely have them.</li><li>• Use standard section names and simple formatting.</li><li>• Certificate proof stays private; only credential details appear on resumes.</li></ul></section></aside>
    </div></section>
  </main>;
}
