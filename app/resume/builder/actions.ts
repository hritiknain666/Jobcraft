...
export async function saveResumeDraft(formData: FormData) {
  ...
  const certificateIds = formData.getAll("certificate_ids").map((value) => String(value)).filter(Boolean);

  if (certificateIds.length) {
    const { data: ownedCertificates } = await supabase.from("certificates").select("id").eq("user_id", user.id).in("id", certificateIds);
  ...
  if (id) {
    const { error } = await supabase.from("resumes").update({ name, structured_data, updated_at: new Date().toISOString() }).eq("id", id).eq("user_id", user.id);
