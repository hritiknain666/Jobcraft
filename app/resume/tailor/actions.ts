...
export async function createTailoredResume(formData: FormData) {
  ...
  const [{ data: job }, { data: profile }, { data: resume }] = await Promise.all([
    ...
    supabase.from("profiles").select("full_name,headline,skills,target_roles").eq("id", user.id).maybeSingle(),
    resumeId ? supabase.from("resumes").select("id,name,parsed_text,structured_data").eq("id", resumeId).eq("user_id", user.id).maybeSingle() : Promise.resolve({ data: null }),
  ]);
  ...
  const relevantSkills = jobSkills.filter((skill) => userSkills.some((own) => own.toLowerCase() === skill.toLowerCase()));
  ...
  const content = {
    ...
    guidance: [
      ...
      relevantSkills.length ? `Prioritize proven skills: ${relevantSkills.join(", ")}.` : "Add only job-relevant skills you genuinely possess.",
