      ...
      <div className="jc-content-wrap">
        ...
        <section className="jc-card p-7 sm:p-9">
          <div className="grid gap-8 xl:grid-cols-[1fr_300px] xl:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-4"><span className="jc-company-square">{companyInitials(job.company)}</span><div><div className="flex flex-wrap items-center gap-2"><p className="jc-eyebrow !text-[10px]">{job.company}</p><span className={`rounded-full px-3 py-1 text-[9px] font-black ${isSample ? "bg-[#f7e0c7] text-[#795939]" : "bg-[#e4f0e9] text-[#278363]"}`}>{isSample ? "SAMPLE ROLE" : "LIVE ROLE"}</span></div><p className="mt-2 text-xs text-[#789087]">{isSample ? "JobCraft prototype listing" : "Aggregated live vacancy"}</p></div></div>
        ...
        <section className="mt-6 grid gap-6 xl:grid-cols-[1fr_330px]">
          ...
          <aside className="space-y-4 xl:sticky xl:top-24 xl:self-start">
            {user ? <>
              <article className="jc-dark-card jc-tool-panel"><p className="jc-eyebrow !text-[#f49a48]">NEXT ACTION</p><h2 className="jc-section-title !text-white">Move this role forward.</h2><div className="mt-5 grid gap-3">
              ...
              <article id="resume-for-role" className="jc-card jc-tool-panel"><p className="jc-eyebrow">RESUME FOR THIS ROLE</p><h2 className="jc-section-title">Choose → tailor → review.</h2><p className="jc-section-subtitle">Keep the facts the same. Change only the emphasis, then review the result before use.</p>{resumes.length ? <form action={createTailoredResume} className="mt-4 grid gap-3"><input type="hidden" name="jobId" value={job.id} /><select name="resumeId" className="jc-input">{resumes.map((resume) => <option key={resume.id} value={resume.id}>{resume.name}{resume.is_primary ? " (Primary)" : ""}</option>)}</select><button className="jc-button-primary">Create role version →</button></form> : <Link href="/resume" className="jc-button-primary mt-4 w-full">Add a resume first →</Link>}</article>
