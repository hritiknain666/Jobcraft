...
import type { JobFacets } from "@/lib/job-sources/facets";

type SearchParams = Record<string, string | undefined>;

export function JobsSearchHero({ params, facets }: { params: SearchParams; facets: JobFacets }) {
  const quickSkills = facets.skills.slice(0, 3);
  const workModes = facets.workModes.length ? facets.workModes : ["On-site", "Hybrid", "Remote"];
      ...
      <div className="relative mx-auto max-w-[1400px] px-5 py-10 sm:px-8 lg:py-12">
        <div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 max-w-xl text-lg leading-8 text-slate-600">Search by role, city, salary, experience, skills and work mode. Suggestions stay current with the active jobs in JobCraft.</p>
          </div>
        ...
        <form className="mt-8 rounded-[28px] border border-slate-200 bg-white p-4 shadow-[0_20px_55px_rgba(15,23,42,.08)]">
          <datalist id="job-title-options">{facets.titles.map((title) => <option key={title} value={title} />)}</datalist>
          <datalist id="job-location-options">{facets.locations.map((location) => <option key={location} value={location} />)}</datalist>
          <datalist id="job-skill-options">{facets.skills.map((skill) => <option key={skill} value={skill} />)}</datalist>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-6">
            <input name="q" list="job-title-options" defaultValue={params.q} placeholder="Role, skill or company" className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 outline-none transition focus:border-violet-400 focus:bg-white lg:col-span-2" />
            <input name="location" list="job-location-options" defaultValue={params.location} placeholder="City" className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 outline-none transition focus:border-violet-400 focus:bg-white" />
            <select name="work_mode" defaultValue={params.work_mode ?? ""} className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5">
            ...
            <input name="skill" list="job-skill-options" defaultValue={params.skill} placeholder="Skill e.g. SQL" className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5" />
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            {quickSkills.length > 0 && <div className="ml-auto hidden gap-2 lg:flex">
              {quickSkills.map((skill) => <Link key={skill} href={`/jobs?skill=${encodeURIComponent(skill)}`} className="rounded-full bg-slate-100 px-3 py-2 text-xs font-black text-slate-600 hover:bg-violet-50 hover:text-violet-700">{skill}</Link>)}
            </div>}
          </div>
        </form>
      </div>
