  ...
  return (
    <div className="p-5 sm:p-6">
      <div className="flex gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#0b1020] text-sm font-black text-white">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`rounded-full px-2.5 py-1 text-[10px] font-black ${isSample ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}
            >
            ...
            {isAdzuna && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[11px] font-black text-slate-500 underline decoration-slate-300 underline-offset-2"
              >
                Jobs by Adzuna ↗
              </a>
            )}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {(job.skills ?? []).slice(0, 6).map((skill: string) => {
          ...
          return (
            <span
              key={skill}
              className={`rounded-lg px-3 py-1.5 text-xs font-black ${matched ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}
            >
              {skill}
              {matched ? " ✓" : ""}
            </span>
        ...
        {!job.skills?.length && (
          <span className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-500">
            Skills may be listed in the description
          </span>
        )}
      </div>
    </div>
