      ...
      {match ? (
        <>
          ...
          <div className="mt-3 h-2 rounded-full bg-slate-200">
            <div className="h-2 rounded-full bg-emerald-500" style={{ width: `${match.score}%` }} />
          </div>
          <p className="mt-2 text-[11px] font-black uppercase tracking-[.08em] text-slate-400">
            {Math.round(match.evidenceCoverage * 100)}% evidence coverage
          </p>
        ...
        <>
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500">
            Add your profile skills to make each role easier to judge.
          </p>
        </>
      )}
      <Link
        href={`/jobs/${jobId}`}
        className="mt-5 block rounded-xl bg-[#0b1020] px-4 py-3 text-center text-sm font-black text-white transition group-hover:bg-violet-600"
      >
        View role →
      </Link>
    </aside>
