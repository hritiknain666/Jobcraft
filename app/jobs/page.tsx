...
export default async function JobsPage({
  ...
  let profile: (JobsListProfile & { full_name?: string | null; headline?: string | null }) | null =
    null;
  if (user) {
  ...
  if (searchTerm)
    query = query.textSearch("search_document", searchTerm, {
      type: "websearch",
      config: "simple",
    });
  ...
  if (skillTerm)
    query = query.textSearch("search_document", skillTerm, { type: "websearch", config: "simple" });
  ...
  if (user && pageJobs.length) {
    const { data: tracked } = await supabase
      ...
      .in(
        "job_id",
        pageJobs.map((job: JobRecord) => job.id),
      );
  ...
  const newTodayCount = pageJobs.filter(
    (job: JobRecord) =>
      job.posted_at &&
      new Intl.DateTimeFormat("en-CA", {
        ...
        month: "2-digit",
        day: "2-digit",
      }).format(new Date(job.posted_at)) === todayKey,
  ).length;
  const strongMatches = profile
    ? pageJobs
        .map((job: JobRecord) => calculateJobsListMatch(job, profile))
        .filter((match) => match && match.score >= 75).length
    : 0;
    ...
    >
      <div className="jc-content-wrap">
        <section className="jc-discover-head">
          ...
          <a href="#job-filters" className="jc-button-secondary">
            ☷ Filters⌄
          </a>
        </section>
        ...
        <div className="jc-card mb-5 flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="jc-eyebrow">FRESH OPPORTUNITIES</p>
            <p className="mt-2 text-sm font-bold text-[#173f33]">
              <span className="jc-serif text-2xl text-[#278363]">{newTodayCount}</span> new role
              {newTodayCount === 1 ? "" : "s"} today
              {profile ? (
                <>
                  {" "}
                  ·{" "}
                  <span className="text-[#278363]">
          ...
          <span className="text-xs leading-5 text-[#789087]">
            Freshness uses India time and the current result page.
          </span>
        </div>
        ...
        <form action="/jobs" className="jc-card jc-search-panel" id="job-filters">
          <div className="jc-search-row">
            <label className="jc-search-field">
              ...
              <input
                ...
                placeholder="Search title, skill, company, or job description"
                aria-label="Search title, skill, company, or job description"
              />
            </label>
            <label className="jc-search-field">
              <LocationIcon />
          ...
          <datalist id="jc-locations">
            {facets.locations.slice(0, 80).map((location) => (
              <option key={location} value={location} />
            ))}
          </datalist>
          <datalist id="jc-skills">
            {facets.skills.slice(0, 120).map((skill) => (
              <option key={skill} value={skill} />
            ))}
          </datalist>
          <details
            className="jc-filter-details"
            open={Boolean(params.skill || params.work_mode || params.salary || params.experience)}
          >
            ...
            <div className="jc-filter-grid">
              ...
              />
              <button className="jc-button-primary" type="submit">
                Apply filters →
              </button>
            </div>
          </details>
        ...
        <div className="jc-results-meta">
          <span>
            <b>{resultCount}</b> role{resultCount === 1 ? "" : "s"} tuned to your direction
          </span>
          <span className="flex items-center gap-2 text-[#278363]">
            ✣ {profile ? "Match engine active" : "Create a profile to activate matching"}
          </span>
        </div>
        {error ? (
          <div className="jc-card p-5 text-red-700">Could not load jobs: {error.message}</div>
        ) : null}
              ...
              <article key={job.id} className="jc-card jc-job-card">
                ...
                >
                  <div className="jc-job-card-top">
                    ...
                    <span className="jc-bookmark" aria-hidden="true">
                      {trackedStatus ? "✓" : "⌑"}
                    </span>
                  </div>
                  <div className="jc-job-source">
                    {job.company}{" "}
                    <span className="ml-2 rounded-full bg-[#e4f0e9] px-2 py-1 text-[9px] normal-case tracking-normal text-[#278363]">
                      Live role
                    </span>
                  </div>
                  <h2 className="jc-job-title">{job.title}</h2>
                  <p className="jc-job-details">
                  ...
                  {match ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {match.matchedSkills.slice(0, 2).map((skill) => (
                        <span key={skill} className="jc-chip !bg-[#e9f4ed] !text-[#285844]">
                          ✓ {skill}
                        </span>
                      ))}
                      {match.missingSkills.slice(0, 1).map((skill) => (
                        <span key={skill} className="jc-chip !bg-[#f8ead9] !text-[#76573b]">
                          Gap · {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="jc-job-card-footer">
                    <div className="jc-job-match">
                      {match
                        ? `${match.score}% match · ${Math.round(match.evidenceCoverage * 100)}% evidence`
                        : "Build profile for match"}
                    </div>
                    <div className="jc-job-age">{postedAge(job.posted_at)}</div>
                  </div>
                </Link>
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                  {user ? (
                    trackedStatus ? (
                      <Link
                        href="/applications"
                        className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#278363] no-underline"
                      >
                        {trackedStatus} ✓
                      </Link>
                    ) : (
                      <form action={saveApplication}>
                        ...
                        <button className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#173f33]">
                          Save role +
                        </button>
                      </form>
                    )
                    ...
                    <Link
                      ...
                      scroll={false}
                      className="text-[10px] font-extrabold uppercase tracking-[.08em] text-[#173f33] no-underline"
                    >
                      Save role +
                    </Link>
                  )}
                  {attribution?.requiredPerListing && attribution.href ? (
                    <a
                      ...
                      className="text-[9px] font-semibold text-[#8a9b95] no-underline"
                    >
                      {attribution.label} ↗
                    </a>
                  ) : null}
        ...
        {!pageJobs.length && !error ? (
          <div className="jc-card mt-5 p-12 text-center text-[#6f887f]">
            No roles matched. Try a wider search.
          </div>
        ) : null}
        {pageJobs.length > 0 ? (
          <p className="mt-5 text-xs leading-6 text-[#789087]">
            JobCraft aggregates live vacancies from multiple sources. Always verify the original
        ...
        {totalPages > 1 && !error ? (
          <nav aria-label="Job result pages" className="jc-pagination">
            {page > 1 ? (
              <Link href={pageHref(params, page - 1)} className="jc-button-secondary">
                ← Previous
              </Link>
            ) : (
              <span className="jc-button-secondary opacity-40">← Previous</span>
            )}
            <span className="text-sm font-bold text-[#718981]">
            ...
            {page < totalPages ? (
              ...
              <span className="jc-button-secondary opacity-40">Next →</span>
            )}
          </nav>
        ) : null}
      </div>
