...
export default async function JobsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  if(user){const{data:p}=await supabase.from("profiles").select("skills,experience_years,city,target_roles,preferred_work_modes").eq("id",user.id).maybeSingle();profile=p??null}
