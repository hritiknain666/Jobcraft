...
export async function createCoverLetter(formData: FormData) {
  ...
  const [{ data: job }, { data: profile }] = await Promise.all([
    ...
    supabase
      .from("profiles")
      .select("full_name,headline,skills,city")
      .eq("id", user.id)
      .maybeSingle(),
  ]);
  ...
  const relevant = (job.skills ?? []).filter((skill: string) =>
    ownSkills.some((own) => own.toLowerCase() === skill.toLowerCase()),
  );
  ...
  const { data, error } = await supabase
    .from("cover_letters")
    .insert({ user_id: user.id, job_id: job.id, title: `${job.title} — ${job.company}`, body })
    .select("id")
    .single();
