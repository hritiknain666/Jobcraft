    ...
    <AuthPageShell mode="login">
      ...
      <form action={login} className="mt-7 space-y-4">
        ...
        <div className="flex justify-end">
          <Link href={`/auth/forgot-password${nextQuery}`} className="jc-text-link">
            Forgot password?
          </Link>
        </div>
      ...
      <div className="mt-5 rounded-2xl border border-[#cfe0d8] bg-[#e9f4ed] p-4">
        ...
        <p className="mt-1 text-xs leading-5 text-[#5f786f]">
          Check spam first. If it still hasn&apos;t arrived, enter the same signup email and resend
          it.
        </p>
        <form action={resendConfirmation} className="mt-3 flex flex-col gap-2 sm:flex-row">
          ...
          <input
            ...
            className="jc-input min-w-0 flex-1 !py-2.5 text-sm"
          />
          <button className="jc-button-secondary !px-4 !py-2.5 text-sm">Resend email</button>
        </form>
      </div>

      <p className="mt-6 text-center text-sm text-[#789087]">
        New to JobCraft?{" "}
        <Link href={`/auth/signup${nextQuery}`} className="font-black text-[#278363]">
          Create an account
        </Link>
      </p>
    </AuthPageShell>
