...
as $$
declare
  run_row record; item record; has_error boolean; is_disabled boolean; is_rate_limited boolean;
  source_name text; display_name_value text; external_prefix text; active_count integer;
  configured_value boolean; enabled_value boolean;
begin
  select triggered_at, finished_at, summary into run_row
  from public.job_refresh_runs where summary is not null order by triggered_at desc limit 1;
  ...
  for item in select key, value from pg_catalog.jsonb_each(run_row.summary) loop
    ...
    if external_prefix is not null then
      select count(*) into active_count from public.jobs
      where source = source_name and external_id like external_prefix || '%'
        and is_active = true and duplicate_of is null;
    else
      select count(*) into active_count from public.jobs
      where source = source_name and is_active = true and duplicate_of is null;
    end if;
    ...
    values(
      ...
      coalesce((item.value->>'fetched')::integer, 0), coalesce((item.value->>'upserted')::integer, 0),
      active_count, case when has_error then 1 else 0 end, item.value->>'error', now()
    )
    on conflict(source_key) do update set
      provider = excluded.provider, display_name = excluded.display_name,
      configured = excluded.configured, enabled = excluded.enabled,
      status = case when is_rate_limited then public.job_source_health.status else excluded.status end,
      last_run_at = excluded.last_run_at,
      last_success_at = case when is_rate_limited then public.job_source_health.last_success_at when excluded.status = 'healthy' then excluded.last_success_at else public.job_source_health.last_success_at end,
      ...
      consecutive_failures = case when is_rate_limited then public.job_source_health.consecutive_failures when excluded.status = 'healthy' then 0 when excluded.status = 'error' then public.job_source_health.consecutive_failures + 1 else public.job_source_health.consecutive_failures end,
