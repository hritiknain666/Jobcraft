...
function indiaRemoteEligible(value: unknown) {
  ...
  if (!v) return true;
  return /\bindia\b|\bworldwide\b|\banywhere\b|\bglobal\b|\bapac\b|\basia\b|\ball locations\b/.test(
    v,
  );
}
...
function inferMode(title: string, description: string, fallback: JobRow["work_mode"] = null) {
  ...
  const negatedRemote =
    /\b(?:not|no)\s+remote\b|\bremote\s+(?:not\s+available|unavailable)\b|\bno\s+wfh\b/i.test(v);
...
async function fetchJson(url: string, init: RequestInit = {}) {
  const response = await fetch(url, {
    ...init,
    headers: { "User-Agent": "JobCraft/1.0 job discovery for candidates", ...(init.headers ?? {}) },
  ...
  try {
    ...
    for (const raw of Array.isArray(payload?.jobs) ? payload.jobs : []) {
      ...
      const title = text(raw?.title, 300),
        company = text(raw?.company_name, 300),
        description = text(raw?.description),
        applyUrl = safeUrl(raw?.url);
      ...
      jobs.push({
        ...
        title,
        company,
        location: text(raw?.candidate_required_location, 300) || "Worldwide",
        work_mode: "Remote",
        experience_min: null,
        experience_max: null,
        salary_min_lpa: null,
        ...
        apply_url: applyUrl,
        is_active: true,
        posted_at: validIso(raw?.publication_date),
      });
    }
...
async function jobicy(): Promise<SourceResult> {
  try {
    ...
    for (const raw of Array.isArray(payload?.jobs) ? payload.jobs : []) {
      ...
      const title = text(raw?.jobTitle, 300),
        company = text(raw?.companyName, 300),
        description = text(raw?.jobDescription),
        applyUrl = safeUrl(raw?.url);
      if (!raw?.id || !title || !company || description.length < 40 || !applyUrl) continue;
      const yearly = String(raw?.salaryPeriod ?? "")
        .toLowerCase()
        .includes("year");
      ...
      jobs.push({
        ...
        title,
        company,
        location: text(raw?.jobGeo, 300) || "Worldwide",
        work_mode: "Remote",
        experience_min: null,
        experience_max: null,
        salary_min_lpa: yearly && inr ? annualInrToLpa(raw?.salaryMin) : null,
        salary_max_lpa: yearly && inr ? annualInrToLpa(raw?.salaryMax) : null,
        skills: skillsFor(
          title,
          description,
        ...
        apply_url: applyUrl,
        is_active: true,
        posted_at: validIso(raw?.pubDate),
      });
    }
    return { source: "Jobicy", jobs };
  } catch (e) {
    return { source: "Jobicy", jobs: [], error: e instanceof Error ? e.message : "Unknown error" };
  }
}
...
function restrictionText(restriction: LocationRestriction) {
  ...
  return String(
    restriction?.alpha2 ?? restriction?.countryCode ?? restriction?.name ?? restriction?.slug ?? "",
  );
}
...
async function himalayas(): Promise<SourceResult> {
  try {
    const payload = await fetchJson(
      "https://himalayas.app/jobs/api/search?country=IN&sort=recent&page=1",
    );
    ...
    for (const raw of rows) {
      ...
      const eligible =
        ...
        restrictions.some(
          (r: LocationRestriction) =>
            /^(in|india)$/i.test(restrictionText(r).trim()) ||
            /\bindia\b/i.test(restrictionText(r)),
        );
      if (!eligible) continue;
      const title = text(raw?.title, 300),
        company = text(raw?.companyName, 300),
        description = text(raw?.description),
        applyUrl = safeUrl(raw?.applicationLink);
      if (!raw?.guid || !title || !company || description.length < 40 || !applyUrl) continue;
      const expiryRaw = raw?.expiryDate;
      const expiry =
        typeof expiryRaw === "number" ? expiryRaw : new Date(String(expiryRaw ?? "")).getTime();
      ...
      jobs.push({
        ...
        experience_min: null,
        experience_max: null,
        salary_min_lpa: inr ? annualInrToLpa(raw?.minSalary) : null,
        salary_max_lpa: inr ? annualInrToLpa(raw?.maxSalary) : null,
        skills: skillsFor(title, description, [
  ...
  try {
    ...
    for (const raw of rows) {
      ...
      const remoteEligible =
        indiaRemoteEligible(location) || tags.some((tag: string) => indiaRemoteEligible(tag));
      if (
        !remoteEligible ||
        (!location &&
          !tags.some((tag: string) => /worldwide|anywhere|global|india|apac|asia/i.test(tag)))
      )
        continue;
      const title = text(raw?.position, 300),
        company = text(raw?.company, 300),
        description = text(raw?.description),
        applyUrl = safeUrl(raw?.url || raw?.apply_url);
      if (
        !raw?.id ||
        title.length < 3 ||
        company.length < 2 ||
      ...
      jobs.push({
        ...
        salary_min_lpa: null,
        salary_max_lpa: null,
        skills: skillsFor(title, description, tags),
        description,
        apply_url: applyUrl,
        is_active: true,
        posted_at: validIso(raw?.date || raw?.epoch),
      });
    }
  ...
  try {
    ...
    const payload = await fetchJson("https://api.theirstack.com/v1/jobs/search", {
      ...
      body: JSON.stringify({
        ...
        discovered_at_gte: discoveredSince,
        is_closed: false,
        limit: 5,
        page: 0,
      }),
    ...
    for (const raw of Array.isArray(payload?.data) ? payload.data : []) {
      const title = text(raw?.job_title, 300),
        company = text(raw?.company, 300),
        description = text(raw?.description),
        applyUrl = safeUrl(raw?.url ?? raw?.source_url);
      if (
        raw?.id === null ||
        raw?.id === undefined ||
        !title ||
        ...
        continue;
      const extras = [
        ...(Array.isArray(raw?.technology_slugs) ? raw.technology_slugs : []),
        ...(Array.isArray(raw?.keyword_slugs) ? raw.keyword_slugs : []),
      ];
      ...
      jobs.push({
        ...
        title,
        company,
        location: text(raw?.long_location ?? raw?.location, 300) || "India",
        work_mode: raw?.remote ? "Remote" : raw?.hybrid ? "Hybrid" : inferMode(title, description),
        experience_min: null,
        experience_max: null,
        salary_min_lpa: currency === "INR" ? annualInrToLpa(raw?.min_annual_salary) : null,
        salary_max_lpa: currency === "INR" ? annualInrToLpa(raw?.max_annual_salary) : null,
        skills: skillsFor(title, description, extras),
        description,
        apply_url: applyUrl,
        is_active: true,
        posted_at: validIso(raw?.date_posted),
      });
    }
    ...
    for (const result of results) {
      ...
      if (result.jobs.length) {
        const { data, error } = await supabase
          .from("jobs")
          .upsert(result.jobs, { onConflict: "source,external_id" })
          .select("id");
        if (error) {
          failed += 1;
          summary[result.source] = {
