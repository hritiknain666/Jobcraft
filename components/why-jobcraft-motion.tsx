...
export default function WhyJobCraftMotion() {
  return (
    <section className="relative overflow-hidden bg-[#090d1f] py-20 text-white sm:py-24">
      ...
      <div className="relative mx-auto max-w-[1380px] px-5 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-[.85fr_1.15fr] lg:items-end">
          <div>
            ...
            <h2 className="mt-4 text-4xl font-black tracking-[-.045em] sm:text-5xl">
              A serious job search should feel clear, not chaotic.
            </h2>
          </div>
          <p className="max-w-2xl text-lg leading-8 text-slate-300">
            JobCraft is built to reduce noise and make the next useful action obvious — from
            discovering a role to tracking the outcome.
          </p>
        </div>
        ...
        <div className="mt-10 overflow-hidden rounded-2xl border border-white/10 bg-white/5 py-4 backdrop-blur">
          <div className="motion-marquee flex w-max gap-3 px-3">
            {[...words, ...words].map((word, i) => (
              <span
                key={`${word}-${i}`}
                className="rounded-full border border-white/10 bg-white/10 px-5 py-2.5 text-sm font-black text-white/90"
              >
                {word}
              </span>
            ))}
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {cards.map(([num, title, text], i) => (
            <article
              key={num}
              className={`motion-float rounded-[24px] border border-white/10 bg-white/[.06] p-6 backdrop-blur-xl ${i % 2 === 0 ? "motion-delay-a" : "motion-delay-b"}`}
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-black tracking-[.16em] text-violet-300">{num}</span>
                <span className="h-2.5 w-2.5 rounded-full bg-violet-400 shadow-[0_0_20px_rgba(167,139,250,.7)]" />
              </div>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[.06] p-6 sm:p-8">
            ...
            <p className="text-xs font-black tracking-[.16em] text-violet-300">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-4">
              {[
                ...
                <div
                  key={label}
                  className="group relative rounded-2xl border border-white/10 bg-black/10 p-4"
                >
                  <p className="text-xs font-black text-white/35">{n}</p>
                  <p className="mt-5 text-lg font-black">{label}</p>
                  {i < 3 && (
                    <span className="absolute -right-2 top-1/2 hidden -translate-y-1/2 text-violet-300 sm:block">
                      →
                    </span>
                  )}
                </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-violet-500/10 p-6 sm:p-8">
            ...
            <p className="mt-4 text-3xl font-black tracking-[-.035em]">
              Less admin. Better decisions. More confident applications.
            </p>
            <p className="mt-4 leading-7 text-slate-300">
              Not more tabs. Not more generic advice. One connected workspace that keeps the
