    ...
    <div className="jc-app-shell">
      <aside className="jc-sidebar">
        <Link href="/dashboard" className="jc-brand" aria-label="JobCraft workspace">
          <span className="jc-brand-mark" aria-hidden="true">
            <NavIcon name="spark" />
          </span>
        ...
        <div className="jc-nav-section jc-tools-section">
          ...
          <nav className="jc-nav-list" aria-label="Career tools">
            {toolNav.map(([key, label, href, icon]) => (
              <NavLink
                key={key}
                active={active === key || (key === "resume" && active === "resume-builder")}
                href={href}
                label={label}
                icon={icon}
                compact
        ...
        <div className="jc-sidebar-bottom">
          <div className="jc-signal-card" aria-label={`Profile strength ${safeStrength}%`}>
            <div className="jc-signal-head">
              <span>PROFILE SIGNAL</span>
              <b>{safeStrength}%</b>
            </div>
            <div className="jc-signal-track">
              <span style={{ width: `${safeStrength}%` }} />
            </div>
            <p>
              {safeStrength >= 85
                ? "Your signal is strong"
                : "Complete My profile to improve matches"}
            </p>
          </div>
          ...
          <div className="jc-user-row">
            ...
            <span className="jc-user-copy">
              ...
              <span>
                {headline?.trim() ||
                  (authenticated ? "JobCraft candidate" : "Activate your career workspace")}
              </span>
            </span>
          </div>
        </div>
        ...
        <header className="jc-topbar">
          ...
          <div className="jc-top-actions">
            <Link
              ...
              className="jc-button-primary !px-3 !py-2 text-[11px]"
              aria-label="Open AI Assistant preview"
            >
              ✣ AI Assistant{" "}
              <span className="ml-1 rounded-full bg-white/20 px-2 py-0.5 text-[8px] uppercase tracking-[.08em]">
                Preview
              </span>
            </Link>
            {!authenticated ? (
              <Link href="/dashboard?auth=login" scroll={false} className="jc-text-link">
                Log in
              </Link>
            ) : null}
            <Link href="/applications" className="jc-bell" aria-label="Applications">
              <NavIcon name="bell" />
              <span />
            </Link>
          </div>
...
function NavLink({
  ...
  return (
    <Link
      href={href}
      className={`jc-nav-item ${active ? "is-active" : ""} ${compact ? "is-compact" : ""}`}
    >
...
function NavIcon({ name }: { name: string }) {
  ...
  if (name === "briefcase")
    return (
      <svg {...common}>
        ...
        <path d="M8 7V5.7C8 4.8 8.8 4 9.7 4h4.6c.9 0 1.7.8 1.7 1.7V7M3 11.5c5.7 2.2 12.3 2.2 18 0" />
      </svg>
    );
  if (name === "user")
    return (
  ...
  if (name === "wand")
    return (
      <svg {...common}>
        <path d="m4 20 11-11M13 4l1 3 3 1-3 1-1 3-1-3-3-1 3-1 1-3ZM18 14l.7 2.1L21 17l-2.3.8L18 20l-.8-2.2L15 17l2.2-.9L18 14Z" />
      </svg>
    );
  if (name === "award")
    return (
