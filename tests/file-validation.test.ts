...
test("accepts files whose extension, MIME and signature agree", async () => {
  const pdf = upload("resume.pdf", "application/pdf", [0x25, 0x50, 0x44, 0x46, 0x2d, 0x31, 0x2e, 0x37]);
...
test("rejects spoofed extensions and MIME types", async () => {
  ...
  const wrongMime = upload("proof.png", "image/jpeg", [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]);
...
test("recognizes DOCX as an Office ZIP package, not an arbitrary ZIP", async () => {
  ...
  ], "resume.docx", { type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" });
  const zip = upload("resume.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", [0x50, 0x4b, 0x03, 0x04]);
